...
MODEL="survpath" # what type of model do you want to train?  transmil_wsi survpath
DIM1=8
DIM2=166
...
LRS=(0.0005)
DECAYS=(0.001)
RESULT_DIR=("mccsdg_train_results_titan_stad")
...
for STUDY in "${STUDIES[@]}"; do
    for lr in "${LRS[@]}"; do
        for decay in "${DECAYS[@]}"; do
            if [ "$STUDY" == "blca" ]; then
                CUDA_VISIBLE_DEVICES=0 python test_specific.py \
