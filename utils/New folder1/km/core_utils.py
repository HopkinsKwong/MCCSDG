...
from models.model_SurvPath_with_mixstyle import SurvPath_with_mixstyle
...
from utils.vis_utils import plot_km_curve_from_scores
...
def _init_model(args):
    ...
    elif args.modality == "survpath_with_mixstyle":

        model_dict = {'omic_sizes': args.omic_sizes, 'num_classes': args.n_classes,
                      'wsi_embedding_dim':args.encoding_dim}


        model = SurvPath_with_mixstyle(**model_dict)
...
def _unpack_data(modality, device, data):
    ...
    elif modality in ["survpath","survpath_with_mixstyle"]:
...
def _train_loop_survival(epoch, model, modality, loader, optimizer, scheduler, loss_fn):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        out, y_disc, event_time, censor, clinical_data_list = _process_data_and_forward(model, modality, device, data)
...
def _summary(dataset_factory, model, modality, loader, loss_fn, survival_train=None):
    ...
    # ��ȡ��ǰʱ�䲢��ʽ��Ϊ�ַ���
    current_time = datetime.now().strftime('%Y%m%d_%H%M%S')  # ��ʽ��������_ʱ����
    
    plot_km_curve_from_scores(all_risk_scores, all_event_times, all_censorships, save_path=f'{current_time}_km_curve.png')
