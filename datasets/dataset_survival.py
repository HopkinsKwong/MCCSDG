...
class SurvivalDataset(Dataset):
    ...
    def _load_wsi_embs_from_path(self, data_dir, slide_ids):
        ...
        for slide_id in slide_ids:
            slide_id = slide_id.rstrip('.svs').split('.')[0]
            # pattern = f"{data_dir}/{slide_id}*_0_512.npy"
            pattern = f"{data_dir}/{slide_id}*_0_1024.npy"
            # print(f"data_dir/slide_id : {data_dir}/{slide_id}")
            wsi_paths = glob.glob(pattern)
            # print(f"wsi_paths:{wsi_paths}")
            wsi_df = np.load(wsi_paths[0], allow_pickle=True)[()]
            wsi_bag = wsi_df['feature']
